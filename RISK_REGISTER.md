...
| R-26 | Runtime誤用migration/schema-owner權限 | 中 | 極高 | 外建runtime login、bounded provisioning、startup privilege proof、owner DSN不進長駐process；`control_state` direct UPDATE 撤銷並改由migration 0019窄化control functions | Security/DB | Mitigated（ADR-016、ADR-035；2026-08-27 real-PG runtime authority probe） |
...
| R-22 | Flatten 在緊急時自行下單但未先暫停/取消/對帳，或重複 flatten 撞 client order id | 中 | 極高 | 六步順序 fail-closed（確認→paused→resolve→cancel→refresh→position 對帳不符即 abort）；`FlattenPriceProvider` seam；`control_state.flatten_generation` 同交易原子遞增為 target_version；SELL-only `RISK_EXIT` 例外與未來 short／BUY-to-cover separate review | Execution | Mitigated（ADR-022、ADR-034；tests/test_control_plane.py flatten 5 案，2026-08-27） |
...
| R-28 | TradingAgents upstream/provider/data drift 破壞 P3 語意、可重播性或隔離邊界 | 中 | 高 | 固定 SHA/dependency lock、semantic-parity/record-replay、graph/prompt/model/provider versions、季度 drift review、free-text fallback fail closed | Research/Security | Open（ongoing drift review；不再是P3 Gate blocker） |
...
| R-30 | long/short 與同日交易放大 gross、borrow、turnover 或虧損追殺風險 | 中 | 極高 | P4第一版short disabled；long/total gross 90%、cash 10%、15檔／單股5%、sector25%、cluster30%、20% turnover、same-day loss reason/evidence gate；任何short需新ADR | Portfolio/Risk | Open（P4 implementation／P5 calibration） |
...
| R-32 | Provider reasoning 參數或 API 類型不一致造成未察覺降級 | 高 | 高 | capability negotiation、requested/effective audit、strict Chat Completions parser、schema/held-out eval、`reasoning_effective=UNKNOWN` 誠實記錄 | Research | Reopened 2026-08-28（route 改 NVIDIA `openai/gpt-oss-120b`）；P3-E 6/6＋P3-F V14 gates 通過且無未察覺降級跡象，pending fresh acceptance |
| R-33 | 反思記憶無限增長、被壓縮器改寫事實或把 future outcome 洩漏給歷史 run | 中 | 高 | immutable raw audit、每日 lineage、週六專用 skill、LLM-visible ≤4,000 行、as-of/time-travel tests | Research/Data | Mitigated（P3-F lineage/bounds/curation已獨立驗收Closed，2026-08-26；P5持續重驗） |
| R-34 | 將provider timeout誤判為程式錯誤而反覆換held-out split，或把單次全綠誤當永久可用性 | 高 | 高 | ADR-033：功能／transport Gate分離、每案最多2 retry、780 attempt cap、backoff+jitter、3-case circuit breaker、rolling 7日≥200 canary | Research/Ops | Partly mitigated（2026-08-28 V14 live 的 8 次 retry 已判定為單一連線層故障爆發（≈2s connect budget 用盡、TRANSIENT、無 RATE_LIMIT），同 payload 重試即成功，非程式缺陷；rolling canary 綁現行 NVIDIA route 仍為 P6 前置） |
| R-35 | 多來源普通fallback把discovery/supplement升權，或以今日revision/symbol污染歷史決策 | 高 | 極高 | ADR-036封閉source role；Alpaca行情authority；ALFRED vintage；SEC/official identity；event-sourced security master；exact-host/rights/schema/as-of gate；衝突禁止新增曝險 | Data/Security | Partly mitigated（P4-A／P4-B／P4-C Accepted／Closed；P4-D～F production composition與P5 time-travel仍Open） |
| R-36 | Alpaca Paper未正確處理拆／合股，造成position/order quantity失真；或錯誤消息觸發不安全強制退出 | 中 | 極高 | ADR-037三層entry block；正式來源＋identity/ratio/date confirmation；cancel/resolve→FULL reconcile→price-protected idempotent exit；late/conflict REVIEW_REQUIRED；fill後P&L/audit/memory | Data/Risk/Execution | Partly mitigated（P4-B quarantine Accepted／Closed；P4-E～P7 exit與short BUY-to-cover未授權） |
| R-37 | 零付費IEX最新行情覆蓋不足，quantity／spread／collar可能偏離完整市場 | 高 | 極高 | feed/entitlement lineage、`LIMITED_MARKET_COVERAGE`、delayed SIP歷史資料、5秒quote、30bps spread、25bps collar、yfinance不得補權 | Data/Risk/Execution | Open（P4 zero-submit可用；P5比較；P7 blocker） |
