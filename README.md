...
`P1-C3 — CI／zero-skip／clean-machine gate` 已通過獨立驗收，P1 Core Gate 已關閉並有遠端 CI 證據。`P2 — Alpaca Paper 執行安全` 於 2026-08-19 通過獨立驗收並經 2026-08-20 的 P2-CUR-001~006 remediation 重驗後曾標示 Closed，但 2026-08-20 的 P2 最終修復驗收（`SEVEN_LENS_P2_FINAL_REMEDIATION_AGENT_PROMPT.md` / ACC-001~009）發現仍有高嚴重度阻斷項，**P2 Gate 已重開（Reopened）**，最終驗收進行中：`AlpacaPaperAdapter`（Paper-only、嚴格解析、官方分頁）、`ExecutionEngine`（`client_order_id` 冪等、`SUBMITTING` 先持久化、timeout→`UNKNOWN` 並持久化 pause、PG exclusive entry linearization）、`TradeUpdateConsumer`（亂序/重播冪等、conflicting fill durable pause）、`Reconciler`（`reconciliation_mismatches` 明細、closed-history、cash checkpoint 與 full-ledger NAV）、`ControlPlane`（`pause`/`resume`/`cancel`/`flatten`）與 `account_baselines` / immutable `account_baseline_revisions`（0008/0009；checksum-compatible 0008 upgrade）為已交付基礎；遠端 `quality-unit` 與 `postgres-integration`（含 0008 與 runtime-role）曾全綠但最終 ACC-001~009 證據尚未齊備。`P2-E` 真實 Alpaca Paper read-only（GET-only）驗證已執行（見 `PROGRESS.md`）；真實下單仍留 `P7` supervised gate，WebSocket transport 與 control shell CLI 依 ADR-019 留 `P6/P7`。
