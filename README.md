...
`P1-C3 — CI／zero-skip／clean-machine gate` 已通過獨立驗收，P1 Core Gate 已關閉並有遠端 CI 證據。`P2 — Alpaca Paper 執行安全` 的歷史 Closed 紀錄因 2026-08-20 final remediation ACC-001~009 再次重開；目前 **P2 Gate Reopened — final acceptance in progress**。ACC-001~008 已在本機修復並通過 fresh locked/real-PostgreSQL gates；ACC-009 只剩 exact final-commit 的遠端 `quality-unit`／`postgres-integration` 證據。已交付基礎包含 Paper-only adapter、exclusive new-entry linearization、durable UNKNOWN/conflicting-fill pause、cash checkpoint + full-ledger NAV、runtime baseline read-only、checksum-compatible 0008→0009，以及 typed `MarkPriceUnavailableError`（任意 `ValueError`/程式缺陷不再降級）。legacy compatibility revision 的 `created_at` 是 legacy `effective_at`／authority-effective timestamp，source-row original `created_at` 保留；不是 migration execution time。`P2-E` 真實 Alpaca Paper GET-only 驗證已執行（見 `PROGRESS.md`）；真實下單仍留 `P7` supervised gate，WebSocket transport 與 control shell CLI 依 ADR-019 留 `P6/P7`。
