"""Generic HTTPS Chat Completions transport with strict JSON and no implicit retry.

One exact POST per attempt to the route carried by the immutable
:class:`AnalysisProviderConfig` snapshot.  The transport verifies TLS against
the configured host, pre-resolves DNS to public numeric addresses, forbids
redirects, proxies, environment routing, retries, and fallbacks, and parses
responses against one exact schema: unknown fields fail closed and outputs are
never authority until the audited invoker commits them.
"""
...
_DNS_IPC_VERSION: Final = "seven-lens-analysis-dns-v1"
_DNS_CLEANUP_SECONDS: Final = 0.2
_MAX_DNS_ADDRESSES: Final = 16
#: Dedicated DNS budget, separate from the short connect budget.  A spawn-based
#: ``getaddrinfo`` child can take longer than a tight TCP connect budget under
#: sustained per-request load; DNS resolution is not connection establishment,
#: so it gets its own bounded deadline (still capped by the total deadline).
...
def _validate_route_path(path: object) -> str:
    if (
...
class RawHttpRequest:
    ...
    def __post_init__(self) -> None:
        if self.method != "POST" or self.scheme != "https":
            raise ValueError("raw provider request route is invalid")
        try:
            validate_route_host(self.host)
        except ValueError:
            raise ValueError("raw provider request route is invalid") from None
        try:
    ...
    def __repr__(self) -> str:
        return (
            "RawHttpRequest(method='POST', route='analysis-route-policy', headers=[REDACTED], "
...
class RawHttpResponse:
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"body_bytes={len(self.body)}, final_route='analysis-route-policy')"
        )


class ChatCompletionsHttpExecutor(Protocol):
    def execute(self, request: RawHttpRequest) -> RawHttpResponse: ...


_FORBIDDEN_RESOLVED_V4_NETWORKS: Final = tuple(
...
_FORBIDDEN_RESOLVED_V6_NETWORKS: Final = tuple(
    ipaddress.ip_network(prefix)
    for prefix in ("::1/128", "::/128", "fc00::/7", "fe80::/10", "ff00::/12")
)


def _resolved_ip_is_public(value: str) -> bool:
    """True only for an address routable on the public internet (never internal scope)."""
    ...
    if address.version == 4:
        return not any(address in network for network in _FORBIDDEN_RESOLVED_V4_NETWORKS)
    if address.is_loopback or address.is_unspecified or address.is_multicast or address.is_reserved:
        return False
    return not any(address in network for network in _FORBIDDEN_RESOLVED_V6_NETWORKS)
...
class ResolvedAddress:
    ...
    def __post_init__(self) -> None:
        ...
        if not _resolved_ip_is_public(host):
            raise ValueError("resolved address is not a public endpoint")
...
class StdlibChatCompletionsHttpExecutor:
    ...
    def execute(self, request: RawHttpRequest) -> RawHttpResponse:
        ...
        try:
            ...
            addresses = self._resolver.resolve(
                request.host,
                min(_DNS_TIMEOUT_SECONDS, request.total_timeout_seconds),
            )
            ...
            return RawHttpResponse(
                ...
                headers=headers,
                body=body,
                final_url=f"https://{request.host}{request.path}",
            )
        except HttpExecutorError:
            raise
        except Exception:
...
#: Provider envelope metadata keys that never carry output authority.  The
#: exact required keys stay fail-closed; anything outside these allowlists is
#: rejected.  Values are validated by `_bounded_metadata_value` only so that a
#: hostile provider cannot smuggle unbounded or secret-bearing material.
_TOP_LEVEL_METADATA_KEYS: Final = frozenset(
    {
        "service_tier",
        "system_fingerprint",
        "prompt_logprobs",
...
_CHOICE_METADATA_KEYS: Final = frozenset({"logprobs", "stop_reason", "token_ids"})
_MESSAGE_METADATA_KEYS: Final = frozenset(
    {
        "reasoning_content",
        "reasoning",
        "tool_calls",
...
_USAGE_METADATA_KEYS: Final = frozenset({"prompt_tokens_details", "completion_tokens_details"})
_SECRET_MARKERS: Final = ("api_key", "apikey", "authorization", "bearer", "password", "secret")
_MAX_METADATA_BYTES: Final = 8_192
_MAX_METADATA_DEPTH: Final = 4


def _bounded_metadata_value(value: object, depth: int = 0) -> None:
    """Accept only bounded, finite, secret-free metadata (never output authority)."""
    ...
    if type(value) is list:
        if len(value) > 32:
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
        for item in value:
            _bounded_metadata_value(item, depth + 1)
        return
    if isinstance(value, dict):
        ...
        try:
            ...
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL) from None
        if len(encoded.encode("utf-8")) > _MAX_METADATA_BYTES or any(
            marker in encoded.lower() for marker in _SECRET_MARKERS
        ):
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
...
def _validate_prompt_token_ids(value: object) -> None:
    """One echo of the prompt tokenization; large but strictly bounded ints."""

    if value is None:
        return
    if type(value) is not list or len(value) > 131_072:
        raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
    if any(type(item) is not int or not 0 <= item <= 2**31 - 1 for item in value):
        raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)


def _validate_prompt_logprobs(value: object) -> None:
    if value is None or type(value) is bool:
        return
...
def _validate_metadata_keys(
    ...
    *,
    token_id_keys: frozenset[str] = frozenset(),
) -> None:
    for key in payload:
        if key not in allowed:
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
        if key in token_id_keys:
            _validate_prompt_token_ids(payload[key])
        elif key == "prompt_logprobs":
            _validate_prompt_logprobs(payload[key])
        else:
            _bounded_metadata_value(payload[key])


def build_chat_completions_request_body(
    ...
    if type(config) is not AnalysisProviderConfig or type(request) is not JsonModelRequest:
        raise ModelTransportError(ModelTransportErrorCode.CONFIG)
    if request.max_output_tokens > config.max_output_tokens:
        raise ModelTransportError(ModelTransportErrorCode.CONFIG)
    # The internal request carries a system/developer/user triple.  Chat Completions
    # providers do not consistently implement the newer ``developer`` role, so the
    # developer text is projected into the system message with a fixed separator.
    # Message order and content are otherwise unchanged.
    system_texts = [
        message.content for message in request.messages if message.role is JsonMessageRole.SYSTEM
    ]
    developer_texts = [
        message.content for message in request.messages if message.role is JsonMessageRole.DEVELOPER
    ]
    wire_messages: list[dict[str, str]] = []
    if system_texts or developer_texts:
        wire_messages.append(
            {"content": "\n\n".join([*system_texts, *developer_texts]), "role": "system"}
        )
    wire_messages.extend(
        {"content": message.content, "role": message.role.value}
        for message in request.messages
        if message.role is not JsonMessageRole.SYSTEM
        and message.role is not JsonMessageRole.DEVELOPER
    )
    wire = {
...
class ChatCompletionsModelTransport:
    """Single-attempt exact-route adapter; outputs are not authority until audited."""
    ...
    def __init__(
        ...
        *,
        ...
        if (
            config.fallback_model_id is not None
    ...
    def execute(self, request: JsonModelRequest) -> JsonModelResponse:
        ...
        if request.max_output_tokens > self._config.max_output_tokens:
            raise ModelTransportError(ModelTransportErrorCode.CONFIG)
        body = build_chat_completions_request_body(self._config, request)
        raw_request = RawHttpRequest(
            method="POST",
            scheme=self._config.scheme,
            host=self._config.host,
            path=self._config.base_path + "/chat/completions",
            headers=(
        ...
        if type(response) is not RawHttpResponse:
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
        if response.final_url != self._config.full_endpoint:
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
    ...
    def _parse_response(self, payload: dict[str, object], raw_body: bytes) -> JsonModelResponse:
        ...
        if not required_outer.issubset(payload):
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
        _validate_metadata_keys(
            {key: value for key, value in payload.items() if key not in required_outer},
            _TOP_LEVEL_METADATA_KEYS,
            token_id_keys=frozenset({"prompt_token_ids"}),
        )
        ...
        if type(payload["created"]) is not int or payload["created"] < 0:
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
        if payload["model"] != self._config.model_id:
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
        ...
        if not required_choice.issubset(choice):
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
        _validate_metadata_keys(
            {key: value for key, value in choice.items() if key not in required_choice},
            _CHOICE_METADATA_KEYS,
            token_id_keys=frozenset({"token_ids"}),
        )
        ...
        if not required_message.issubset(message):
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
        _validate_metadata_keys(
            {
                key: value
                for key, value in message.items()
                if key not in required_message and key not in {"reasoning_content", "reasoning"}
            },
            _MESSAGE_METADATA_KEYS,
        )
        for reasoning_key in ("reasoning_content", "reasoning"):
            if reasoning_key in message:
                reasoning_value = message[reasoning_key]
                if reasoning_value is not None and (
                    type(reasoning_value) is not str
                    or len(reasoning_value.encode("utf-8")) > self._config.response_byte_cap
                ):
                    raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
        if message["role"] != "assistant" or type(message["content"]) is not str:
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
        content = message["content"]
        if not content or len(content.encode("utf-8")) > self._config.response_byte_cap:
            raise ModelTransportError(ModelTransportErrorCode.SCHEMA)
        ...
        if not required_usage.issubset(usage):
            raise ModelTransportError(ModelTransportErrorCode.PROTOCOL)
        _validate_metadata_keys(
            {key: value for key, value in usage.items() if key not in required_usage},
