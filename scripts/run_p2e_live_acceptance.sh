...
if [[ "${SEVEN_LENS_P2E_LIVE:-}" != "1" ]]; then
    echo "ERROR: set SEVEN_LENS_P2E_LIVE=1 only after authorizing real Alpaca Paper GET requests." >&2
    exit 1
fi
...
if ! command -v uv >/dev/null 2>&1; then
    echo "ERROR: uv is required for P2-E live acceptance." >&2
    exit 1
fi
...
owner_label="seven-lens.p2e.owner"
container_id=""
...
cleanup() {
    ...
    if [[ "$candidate_id" =~ ^[0-9a-f]{64}$ ]]; then
        ...
        actual_label="$(docker inspect \
            --format '{{ index .Config.Labels "seven-lens.p2e.owner" }}' \
...
if [[ ! "$server_version_num" =~ ^[0-9]+$ ]] \
    ...
    echo "ERROR: P2-E live acceptance requires PostgreSQL major version 16." >&2
    exit 1
fi
...
SEVEN_LENS_P2E_LIVE=1 \
    uv run --locked pytest \
        tests/integration/test_p2e_readonly_live.py::test_live_read_only_verification_persists_evidence \
        -ra --tb=short
