-- 0022 down: restore the 0012 Agnes-only route identity.
--
-- The down migration fails closed when any generic analysis-route row exists:
-- such rows are real production audit history and must never be truncated,
-- rewritten, or silently re-labeled as Agnes-only.

DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM public.model_call_claims WHERE provider <> 'AGNES')
       OR EXISTS (SELECT 1 FROM public.model_call_audits WHERE provider <> 'AGNES') THEN
        RAISE EXCEPTION
            'generic analysis-route rows exist; down migration is refused'
            USING ERRCODE = '55000';
    END IF;
END;
$$;

ALTER TABLE public.model_call_claims DROP CONSTRAINT model_call_claims_route_identity_check;
ALTER TABLE public.model_call_audits DROP CONSTRAINT model_call_audits_route_identity_check;

ALTER TABLE public.model_call_claims DROP COLUMN route_config_hash;
ALTER TABLE public.model_call_audits DROP COLUMN route_config_hash;

ALTER TABLE public.model_call_claims ADD CONSTRAINT model_call_claims_provider_check
    CHECK (provider = 'AGNES');
ALTER TABLE public.model_call_claims ADD CONSTRAINT model_call_claims_model_check
    CHECK (model = 'agnes-2.5-flash');
ALTER TABLE public.model_call_claims ADD CONSTRAINT model_call_claims_endpoint_policy_id_check
    CHECK (endpoint_policy_id = 'p3e-agnes-2.5-flash-only-v1');

ALTER TABLE public.model_call_audits ADD CONSTRAINT model_call_audits_provider_check
    CHECK (provider = 'AGNES');
ALTER TABLE public.model_call_audits ADD CONSTRAINT model_call_audits_model_check
    CHECK (model = 'agnes-2.5-flash');
ALTER TABLE public.model_call_audits ADD CONSTRAINT model_call_audits_endpoint_policy_id_check
    CHECK (endpoint_policy_id = 'p3e-agnes-2.5-flash-only-v1');

ALTER TABLE public.memory_curation_audits DROP CONSTRAINT memory_curation_audits_model_id_check;
ALTER TABLE public.memory_curation_audits ADD CONSTRAINT memory_curation_audits_model_id_check
...
DELETE FROM public.schema_migrations WHERE version = 22;
