-- 0025 down: restore the pre-0025 P4-C monthly-universe time contract.
--
-- Re-issues the 0024 authority functions (exact-date universe binding and the
-- pinned known_at invariant) and restores the original as_of CHECK constraint
-- name, so the schema and function bodies match revision 0024 exactly.

-- Restore the 0024 as_of CHECK constraint name and definition.
ALTER TABLE public.universe_snapshots
    DROP CONSTRAINT universe_snapshots_as_of_open_session_check;
ALTER TABLE public.universe_snapshots
    ADD CONSTRAINT universe_snapshots_as_of_check
    CHECK (EXTRACT(DAY FROM as_of) = 1);
